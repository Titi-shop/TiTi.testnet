...
export async function POST(req: Request) {
  try {
    ...
    if (!pi_uid)
      return NextResponse.json({ success: false, error: "Thiếu pi_uid" }, { status: 400 });

    const profile = { pi_uid, displayName, email, phone, address, updatedAt: new Date().toISOString() };
