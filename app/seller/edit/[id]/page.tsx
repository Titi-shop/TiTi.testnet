...
export default function EditProfilePage() {
  ...
  const [info, setInfo] = useState({
    pi_uid: "",
    appName: "",          // ← Biệt danh hiển thị trong app
    email: "",
    phoneCode: "+00",
    phone: "",
    address: "",
    ...
    fetch(`/api/profile?username=${encodeURIComponent(username!)}`)
      ...
      .then((data) => {
        ...
        setInfo((prev) => ({
          ...prev,
          pi_uid: data.pi_uid || "",
          appName: data.appName || "",       // ← KHÔNG BAO GIỜ GÁN EMAIL / DISPLAYNAME
          email: data.email || "",
          phone: data.phone || "",
  ...
  const handleSave = async () => {
    ...
    try {
      const body = {
        ...info,
        username: user.username,     // khóa định danh (KHÔNG HIỂN THỊ)
        displayName: user.displayName, // tên thật Pi Network (KHÔNG SỬA)
        avatar,
      };
    ...
    <main className="min-h-screen bg-gray-100 pb-32 relative">
      ...
      <div className="max-w-md mx-auto bg-white rounded-xl shadow-lg mt-12 p-6">
        ...
        <h1 className="text-center text-lg font-semibold text-gray-800 mb-4">
          {user.displayName}   {/* Hiển thị tên PI thật */}
        </h1>
        ...
        <div className="space-y-4">
          ...
          {/* Nickname */}
          <div>
            <label className="block text-sm text-gray-700 mb-1">Biệt danh trong app</label>
            <input
              type="text"
              className="w-full border px-3 py-2 rounded"
              value={info.appName}
              onChange={(e) => setInfo({ ...info, appName: e.target.value })}
              placeholder="Nhập biệt danh tuỳ thích"
            />
          </div>
