  ...
  const fetchOrders = async () => {
    try {
      ...
      const filtered = (data || []).filter(
        (o: Order) =>
          o.status === t.return_orders || o.status === "Hoàn lại"
      );
      setOrders(filtered);
    } catch {
    ...
    <main className="min-h-screen max-w-4xl mx-auto p-4 pb-24 bg-gray-50">
      ...
      <div className="flex items-center mb-4">
        ...
        <h1 className="text-xl font-semibold text-gray-800">
          ↩️ {t.return_orders || "Đơn hàng hoàn lại"}
        </h1>
      </div>
      ...
      {orders.length === 0 ? (
        <p className="text-center text-gray-500">
          {t.no_return_orders || "Không có đơn hoàn lại."}
        </p>
      ) : (
        <div className="space-y-3">
          {orders.map((o) => (
            ...
            >
              ...
              <p>📦 <b>{t.status || "Trạng thái"}:</b> {t.return_orders || "Hoàn lại"}</p>
            </div>
          ))}
        </div>
      )}

      {/* ===== Khoảng trống để tránh che bởi footer ===== */}
